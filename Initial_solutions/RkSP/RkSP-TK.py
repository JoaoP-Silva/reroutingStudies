...
def run(network, begin, end, interval, t, output, probability, vnumber, k_paths, threshold, level, urgency, seed):
...
def start_simulation(sumo, scenario, network, begin, end, interval, output, vnumber, time, probability, k_paths, threshold, level, urgency, seed):
    ...
    try:     
        ...
        run(network, begin, end, interval, time, output, probability, vnumber, k_paths, threshold, level, urgency, seed)
...
def main():
    ...
    parser.add_option("-o", "--output", dest="output", default="Outputs_RkSP-TK/reroute_rksp-TK.xml", help="The XML file at which the output must be written [default: %default]", metavar="FILE")
    parser.add_option("-l", "--logfile", dest="logfile", default="Rksp-tk_log.txt", help="log messages to logfile [default: %default]", metavar="FILE")
    ...
    parser.add_option("--threshold", dest="threshold",type="float", default= 0.5 ,action="store", help="A threshold to set if a road segment is considered congested [default: %default]", metavar="FILE")
    ...
    start_simulation(options.command, options.scenario, options.network, options.begin, options.end, options.interval, options.output, options.vnumber, options.time, options.probability, options.k_paths, options.threshold, options.level, options.urgency, options.seed)
