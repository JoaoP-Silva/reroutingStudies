...
def runEBkSP(num, scenario, outputDir):
    ...
    while(count < len(reroute)):
        ...
        while(i < num):
            ...
            os.system("python %s.py -c sumo.exe -s %s/sim.sumocfg -n %s/sim.net.xml -k 3 -i 900 -b 1000 -e 10000 -d %d -o %s -l logs/%s.log"%(reroute[count], scenario, scenario, i, output, reroute[count]))
            i = i+1
